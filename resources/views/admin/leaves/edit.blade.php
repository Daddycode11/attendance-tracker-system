...
@section('breadcrumb','Edit Leave')

@section('content')
<div class="page-header">
    <div>
        <h1>Edit Leave</h1>
        <p>Update leave record for <strong>{{ $leave->employee->name }}</strong>.</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.leaves.update', $leave) }}">
        ...
        <div class="card">
            <div class="card-header">
                <span class="card-title"><i class="fa-solid fa-calendar-pen" style="color:var(--accent);"></i> &nbsp;Leave Details</span>
            </div>
            <div class="card-body" style="padding:20px;">
                <div class="form-group" style="margin-bottom:16px;">
                    ...
                    <input type="text" class="form-control" value="{{ $leave->employee->name }} ({{ $leave->employee->employee_id }})" readonly
                ...
                <div class="form-group" style="margin-bottom:16px;">
                    ...
                    <select name="leave_type" class="form-control" required
                        ...
                        <option value="{{ $t }}" {{ old('leave_type', $leave->leave_type) == $t ? 'selected' : '' }}>{{ $t }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group" style="margin-bottom:16px;">
                    ...
                    <textarea name="reason" class="form-control" rows="3" placeholder="Optional reason…"
                        style="width:100%;padding:10px 14px;border:1.5px solid var(--border);border-radius:8px;font-size:.88rem;font-family:inherit;resize:vertical;">{{ old('reason', $leave->reason) }}</textarea>
                </div>
                ...
                <div class="form-group" style="margin-bottom:22px;">
                    ...
                    <select name="status" class="form-control" required
                        ...
                        <option value="{{ $s }}" {{ old('status', $leave->status) == $s ? 'selected' : '' }}>{{ $s }}</option>
                        @endforeach
                    </select>
                </div>
